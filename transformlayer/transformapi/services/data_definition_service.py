class Data_Definition_Service:
    ...
    def __get_services_total(params, services, service_types):
    ...
    ####    Expects:
    ####        services - fact service data table
    def __get_undup_hh_total(params, services, service_types):
        return services.drop_duplicates(subset = 'research_family_key', inplace = False)
    
    ## 3
    def __get_undup_indv_total(params, services, service_types):
        return "__get_undup_indv_total"
    
    ## 4
    def __get_services_per_uhh_avg(params, services, service_types):
        return "__get_services_per_uhh_avg"
    
    ## 5
    def __get_hh_wminor(params, services, service_types):
        return "__get_hh_wminor"
    
    ## 6
    def __get_hh_wominor(params, services, service_types):
        return "__get_hh_wominor"
    
    ## 7
    def __get_hh_total(params, services, service_types):
        return "__get_hh_total"
    
    ## 8
    def __get_indv_sen_hh_wminor(params, services, service_types):
        return "__get_indv_sen_hh_wminor"
    
    ## 9
    def __get_indv_sen_hh_wominor(params, services, service_types):
        return "__get_indv_sen_hh_wominor"
    
    ## 10
    def __get_indv_sen_total(params, services, service_types):
        return "__get_indv_sen_total"
    
    ## 11
    def __get_indv_adult_hh_wminor(params, services, service_types):
        return "__get_indv_adult_hh_wminor"
    
    ## 12
    def __get_indv_adult_hh_wominor(params, services, service_types):
        return "__get_indv_adult_hh_wominor"
    
    ## 13
    def __get_indv_adult_total(params, services, service_types):
        return "__get_indv_adult_total"
    
    ## 14
    def __get_indv_child_hh_wminor(params, services, service_types):
        return "__get_indv_child_hh_wminor"
    
    ## 15
    def __get_indv_child_hh_wominor(params, services, service_types):
        return "__get_indv_child_hh_wominor"
    
    ## 16
    def __get_indv_child_total(params, services, service_types):
        return "__get_indv_child_total"
    
    ## 17
    def __get_indv_total_hh_wminor(params, services, service_types):
        return "__get_indv_total_hh_wminor"
    
    ## 18
    def __get_indv_total_hh_wominor(params, services, service_types):
        return "__get_indv_total_hh_wominor"
    
    ## 19
    def __get_indv_total(params, services, service_types):
        return "__get_indv_total"
    
    ## 20
    def __get_hh_wsenior(params, services, service_types):
        return "__get_hh_wsenior"
    
    ## 21
    def __get_hh_wosenior(params, services, service_types):
        return "__get_hh_wosenior"
    
    ## 22
    def __get_hh_grandparent(params, services, service_types):
        return "__get_hh_grandparent"

    ## error, none
    def get_data_def_error(params, services, service_types):
